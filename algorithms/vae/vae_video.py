...
class VAEVideo(BasePytorchAlgo):
    """
    A VAE model for encoding and decoding video data.
    Used for preprocessing video data into a latent space.
    """
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.latent_type = cfg.latent_type
        ...
        latents_to_save = (
            rearrange(
                (
                    ...
                    else latent_dist.parameters.to(torch.float16)
                ),
                "(b f) c h w -> b f c h w",
                b=batch_size,
            )
    ...
    def _rearrange_and_unnormalize(
        ...
        videos = rearrange(videos, "(b f) c h w -> f b c h w", b=batch_size)
        return videos
