...
from algorithms.vae.autoencoder_kl.model import Encoder, Decoder
...
from algorithms.vae.common.losses import LPIPSWithDiscriminator
...
class TrainableAutoencoderKL(pl.LightningModule):
    def __init__(
        self,
        cfg: DictConfig,
    ):
...
class AutoencoderKL(VAE):
    """
    Pretrained AutoencoderKL model that can be used to encode and decode images.
    ...
    @classmethod
    def _from_pretrained_custom(cls, path: str) -> "AutoencoderKL":
        ...
        if "cfg" not in checkpoint:
            checkpoint["cfg"] = OmegaConf.load(
                "configurations/algorithm/autoencoder_kl.yaml"
            )
    ...
    def _from_pretrained_diffuser(cls, path: str, **kwargs) -> VAE:
        vae = DiffuserAutoencoderKL.from_pretrained(path, **kwargs)
...
def diffuser_to_custom(vae: DiffuserAutoencoderKL) -> VAE:
    """
    Modify DiffuserAutoencoderKL to be compatible with VAE abstract class
    """
