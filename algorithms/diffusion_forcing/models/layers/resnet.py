...
from algorithms.diffusion_forcing.models.utils import zero_module
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        wrap = zero_module if version == "v1" else lambda x: x

        self.out_layers = nn.Sequential(
            nn.GroupNorm(num_groups=groups, num_channels=dim_out, eps=eps),
            nn.SiLU(),
            *([nn.Dropout(dropout)] if version == "v1" else []),
            wrap(nn.Conv3d(dim_out, dim_out, kernel_size=(1, 3, 3), padding=(0, 1, 1))),
        )
