...
def compute_stats(feats: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    if feats.shape[0] == 1:  # temporary fix for single batch size
...
# TODO: Replace the usage of this class with `FrechetVideoDistance`.
class DeprecatedFrechetVideoDistance(nn.Module):
    def __init__(self):
        super().__init__()
        detector_url = (
